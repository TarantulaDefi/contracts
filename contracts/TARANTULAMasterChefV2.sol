...
// MasterChefV2 is the master of TAL. He can make TAL and he is a fair guy.
//
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function safetarantulaTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > tarantulaBal) {
            transferSuccess = tarantula.transfer(_to, tarantulaBal);
        } else {
            transferSuccess = tarantula.transfer(_to, _amount);
        }
    ...
    function updateEmissionRate(uint256 _tarantulaPerBlock) public onlyOwner {
        massUpdatePools();
        tarantulaPerBlock = _tarantulaPerBlock;
		emit UpdateEmissionRate(msg.sender, _tarantulaPerBlock);
    }
}
